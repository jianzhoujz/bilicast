// ==UserScript==
// @name         BiliCastHelper
// @namespace    https://github.com/yourname/BiliCastHelper
// @version      0.3.2
...
(function () {
  ...
  function getLocalToken() {
    try { return GM_getValue(TOKEN_KEY, '') || ''; } catch (_) { return ''; }
  }
  function setLocalToken(token) {
    try { GM_setValue(TOKEN_KEY, token || ''); } catch (_) {}
  }
  ...
  function requestLocalApi(path, { method = 'GET', payload = null, withToken = false, timeout = 12000 } = {}) {
  ...
  try {
    GM_registerMenuCommand('设置 BiliCast Token', () => {
      const cur = getLocalToken();
      const v = prompt('粘贴菜单栏 App 里的 token：', cur);
      if (v == null) return;
      setLocalToken(v.trim());
      showToast('Token 已保存。', 'ok');
    });
    GM_registerMenuCommand('清除 BiliCast Token', () => {
      setLocalToken('');
  ...
  console.info('[BiliCastHelper] v0.3.2 loaded; pathname=' + location.pathname +
